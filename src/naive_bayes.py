...
class NaiveBayes:
    ...
    def estimate_class_priors(self, labels: torch.Tensor) -> Dict[int, torch.Tensor]:
        ...
        counts: torch.Tensor = torch.bincount(labels.to(torch.int64))  # Convert labels to int
        nsamples: int = labels.shape[0] 
        class_priors: Dict[int, torch.Tensor] = {i: counts[i].float() / nsamples for i in range(len(counts))}
    ...
    def estimate_conditional_probabilities(
        ...
        nclasses: int = int(torch.max(labels).item()) +1
        class_word_counts: Dict[int, torch.Tensor] = {}
        
        for c in range(nclasses):
            mask = (labels == c)  
            class_features = features[mask] 
            word_counts = torch.sum(class_features, dim=0) + delta 
            total_words = torch.sum(word_counts) 
            class_word_counts[c] = word_counts / total_words  # Normalize probabilities
